...
fn test_list_certificates() {
    let setup = setup_mock_for("/1385/domains/example.com/certificates", "listCertificates/success", "GET");
    let client = setup.0;
    let account_id = 1385;
    ...
    let response = client.certificates().list_certificates(account_id, String::from(domain), sort, paginate).unwrap();
    ...
    assert_eq!("2020-09-16T19:30:07Z", certificate.expires_at.to_owned().unwrap());
    assert_eq!("2020-09-16", certificate.expires_on.to_owned().unwrap());
}

#[test]
fn test_get_certificate() {
    let setup = setup_mock_for("/1385/domains/example.com/certificates/101967", "getCertificate/success", "GET");
    let client = setup.0;
    let account_id = 1385;
    let domain = "example.com";
    let certificate_id = 101967;

    let certificate = client.certificates().get_certificate(account_id, String::from(domain), certificate_id).unwrap().data.unwrap();
    ...
    assert_eq!("2020-09-16T18:10:13Z", certificate.expires_at.to_owned().unwrap());
    assert_eq!("2020-09-16", certificate.expires_on.to_owned().unwrap());
}

#[test]
fn test_download_certificate() {
    let setup = setup_mock_for("/1385/domains/example.com/certificates/101967/download", "downloadCertificate/success", "GET");
    let client = setup.0;
    let account_id = 1385;
    let domain = "example.com";
    let certificate_id = 101967;

    let download = client.certificates().download_certificate(account_id, String::from(domain), certificate_id).unwrap().data.unwrap();
...
fn test_get_certificate_private_key() {
    let setup = setup_mock_for("/1385/domains/example.com/certificates/101967/private_key", "getCertificatePrivateKey/success", "GET");
    let client = setup.0;
    let account_id = 1385;
    let domain = "example.com";
    let certificate_id = 101967;

    let certificate_private_key = client.certificates().get_certificate_private_key(account_id, String::from(domain), certificate_id).unwrap().data.unwrap();
...
fn test_purchase_letsencrypt_certificate() {
    let setup = setup_mock_for("/1385/domains/example.com/certificates/letsencrypt", "purchaseLetsencryptCertificate/success", "POST");
    let client = setup.0;
    let account_id = 1385;
    let domain = "example.com";
    ...
    let letsencrypt = client.certificates().purchase_letsencrypt_certificate(account_id, String::from(domain), payload).unwrap().data.unwrap();
...
fn test_issue_letsencrypt_certificate() {
    let setup = setup_mock_for("/1385/domains/example.com/certificates/letsencrypt/101967/issue", "issueLetsencryptCertificate/success", "POST");
    let client = setup.0;
    let account_id = 1385;
    let domain = "example.com";
    let certificate_id = 101967;

    let certificate = client.certificates().issue_letsencrypt_certificate(account_id, String::from(domain), certificate_id).unwrap().data.unwrap();
...
fn test_purchase_letsencrypt_certificate_renewal() {
    let setup = setup_mock_for("/1385/domains/example.com/certificates/letsencrypt/101967/renewals", "purchaseRenewalLetsencryptCertificate/success", "POST");
    let client = setup.0;
    let account_id = 1385;
    let domain = "example.com";
    let certificate_id = 101967;
    ...
    let letsencrypt_renewal = client.certificates().purchase_letsencrypt_certificate_renewal(account_id, String::from(domain), certificate_id, payload).unwrap().data.unwrap();
...
fn test_issue_letsencrypt_certificate_renewal() {
    let setup = setup_mock_for("/1385/domains/example.com/certificates/letsencrypt/101967/renewals/12121/issue", "issueRenewalLetsencryptCertificate/success", "POST");
    let client = setup.0;
    let account_id = 1385;
    let domain = "example.com";
    let certificate_id = 101967;
    ...
    let renewal = client.certificates().issue_letsencrypt_certificate_renewal(account_id, String::from(domain), certificate_id, certificate_renewal_id).unwrap().data.unwrap();
