...
impl Domains<'_> {
    ...
    ///
    /// let client = new_client(true, String::from("AUTH_TOKEN"));
    /// let payload = DomainCreationPayload {
    ///         name: String::from("example-beta.com")
    ///     };
    /// let domains_response = client.domains().create_domain(1234, payload);
    /// ```
    ///
    /// # Arguments
    ///
    /// `account_id`: The account ID
    /// `payload`: The `DomainCreationPayload` to be sent to the DNSimple API
    pub fn create_domain(&self, account_id: u64, payload: DomainCreationPayload) -> DNSimpleResponse<DomainData> {
        let path = format!("/{}/domains", account_id);
